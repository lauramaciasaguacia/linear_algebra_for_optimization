...
# In this file we perform the steps required to solve question 2a with normalised clustering. 
# The code is very similar to the one for normalised, but there are of course some small differences
...
def normalize(X): #Normalize the data. All entry will be contained between 0 and 1
...
def Boxplots(array,ClusterList,NumberOfClusters,head): 
    for i in range(array.shape[1]):
        ...
        plt.xlabel(head[i])
        plt.savefig(f'2AN{i}.png')  #saves them in the folder
        plt.close()
...
def scatter(array,ClusterList,NumberOfClusters,head): #We are looking if there are any 2-dim dependences between the data
    ...
    for i in range(array.shape[1]):
        for j in range(i+1,array.shape[1]):
            ...
            plt.xlabel(head[i])
            plt.ylabel(head[j])
            plt.savefig(f'2AN{i}vs{j}.png')  # saves them in the folder
            plt.close()
...
D=np.zeros(diagonal.shape)        
#We need to find D = diagonal^(-1/2), but since it is a diagonal matrix, we can do:

for i in range(D.shape[0]):              
    D[i,i] = 1/np.sqrt(diagonal[i,i])


lap = np.identity(similar.shape[0]) - D @ (similar @ D) #Construct the corresponding Laplacian
...
# We have no idea why, but for this Laplacian it's really slow 
# It takes more than 30 mins to compute this even though it is the exact same command
# as for the unnormalised on. Probably the matrix is not nice and sparse anymore
# However, we couldn't find a faster alternative.
...
for i in range(10):
    ...
    #The cluster_allocation is a list of 4000 entries where every entry is either 0,1 or 2
    ...
    #and that is implemented below: clusterlist is a partition of [3999] in 3 clusters
